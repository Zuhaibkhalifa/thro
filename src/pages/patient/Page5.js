...
class Page5 extends React.Component {
  ...
  try {
     axios.get('http://thrombolink.com/server/api/patient/page5LoadData',{
      headers: headers
    })
    .then((response) => {
...
<div>
        ...
        <div className="bg-light blue-box"> {/* white box */}
          ...
          <form className="p-5" action="#!">
            ...
            <br /><br />
            <p className="blue"><b>Please specify the type of condition</b></p>
            <label className="radio-inline blue">Deep vein Thrombosis</label>
            <input type="checkbox" name="optradio" id="optradio1"  className="pull-right" 
            onClick={this.onRadioBtn1}  onChange={(e)=>this.setState({q1_ans:'Deep vein Thrombosis',chkVal:1})} 
            />

<div id="toggle1">
<p className="blue"><b>If So, how long ago</b></p>
              <label className="radio-inline blue">Less than 1 month ago</label>
              <input type="radio" name="dvt1" className="pull-right" 
              onChange={(e)=>this.setState({q1_sub_q1_ans:'Less than 1 month ago'})}
              />
              <br />
              <label className="radio-inline blue">Between 1 and 3 months ago</label>
              <input type="radio" name="dvt1" className="pull-right" 
               onChange={(e)=>this.setState({q1_sub_q1_ans:'Between 1 and 3 months ago'})}
              />
              <br />
              <label className="radio-inline blue">More than 3 months ago</label>
              <input type="radio" name="dvt1" className="pull-right" 
              onChange={(e)=>this.setState({q1_sub_q1_ans:'More than 3 months ago<'})}
              />
              <br />
  ...
  <div className="text-danger"> {this.state.error2!=='' ? this.state.error2: ''}</div>
            ...
            <input type="checkbox" name="optradio" id="optradio2"  className="pull-right" 
        onClick={this.onRadioBtn2} onChange={(e)=>this.setState({q2_ans:"Pulmonary Embolis",chkVal:1})}
            />
      
<div id="toggle2">
<p className="blue"><b>If So, how long ago</b></p>
              <label className="radio-inline blue">Less than 1 month ago</label>
              <input type="radio" name="pe" className="pull-right" 
               onChange={(e)=>this.setState({q1_sub_q2_ans:'Less than 1 month ago'})}
              />
              <br />
              <label className="radio-inline blue">Between 1 and 3 months ago</label>
              <input type="radio" name="pe" className="pull-right" 
            onChange={(e)=>this.setState({q1_sub_q2_ans:'Between 1 and 3 months ago'})}
              />
              <br />
              <label className="radio-inline blue">More than 3 months ago</label>
              <input type="radio" name="pe" className="pull-right" 
               onChange={(e)=>this.setState({q1_sub_q2_ans:'More than 3 months ago'})}
              />
              <br />
              <label className="radio-inline blue">Not Sure</label>
              <input type="radio" name="pe" className="pull-right" 
...
<div className="text-danger"> {this.state.error3!=='' ? this.state.error3: ''}</div>
            ...
            <br />
            <label className="radio-inline blue">Not Sure</label>
            <input type="checkbox" name="optradio" onClick={this.mainOption}   id="mainoption313" className="pull-right" 
