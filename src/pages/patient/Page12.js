...
class Page12 extends React.Component {
  constructor(props) {
    ...
    try {
       axios.get('http://thrombolink.com/server/api/patient/page12LoadData',{
        headers: headers
      })
      .then((response) => {
...
<div>
  ...
  <div className="bg-light blue-box">
    ...
    <form className="p-5" action="#!">
      ...
      <input type="radio" name="optradio" className="pull-right" 
       onClick={this.showhide}
       value="Life Labs" defaultChecked={this.state.q1_ans === "Life Labs"}  onChange={(e)=>this.setState({q1_ans:'Life Labs'})}
       />
      <br />
      <label className="radio-inline blue">Gamma Dyna-care</label>
      <input type="radio" name="optradio" className="pull-right" 
       onClick={this.showhide}
       value="Gamma Dyna-care" defaultChecked={this.state.q1_ans === "Gamma Dyna-care"}  onChange={(e)=>this.setState({q1_ans:'Gamma Dyna-care'})}
       />
      <br />
      <label className="radio-inline blue">Hamilton Health Science</label>
      <input type="radio" name="optradio" className="pull-right" 
       onClick={this.showhide}
       value="Hamilton Health Science" defaultChecked={this.state.q1_ans === "Hamilton Health Science"}  onChange={(e)=>this.setState({q1_ans:'Hamilton Health Science'})}
       />
      <br />
      <label className="radio-inline blue">St. Josesph's Healthcare</label>
      <input type="radio" name="optradio" className="pull-right" 
       onClick={this.showhide}
       value="St. Josesph's Healthcare" defaultChecked={this.state.q1_ans === "St. Josesph's Healthcare"}  onChange={(e)=>this.setState({q1_ans:"St. Josesph's Healthcare"})}
       />
      <br />
      <label className="radio-inline blue">Other</label>
      <input type="radio" name="optradio" className="pull-right" 
       onClick={this.showhide}
       value="Other" defaultChecked={this.state.q1_ans === "Other"} 
       id="other_show_hide"  onChange={(e)=>this.setState({q1_ans:'Other'})}
       />
