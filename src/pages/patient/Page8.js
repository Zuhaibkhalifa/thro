...
class Page8 extends React.Component {
  constructor(props) {
    ...
    try {
       axios.get('http://thrombolink.com/server/api/patient/page8LoadData',{
        headers: headers
      })
      .then((response) => {
...
submitForm() {
  if(!$("input[name='optradio']:checked").val()){
    ...
    } else if(document.getElementById("pradaxa_rdo").checked === true && !$("input[name='opt_pradaxa_dosage']:checked").val()){
  
      this.setState({error1:''});
     this.setState({error2:'This field is required'});
   
    } else if(document.getElementById("xarelto_rdo").checked === true && !$("input[name='opt_xarelto_dosage']:checked").val()){
  
      this.setState({error1:''});
      this.setState({error2:''});
     this.setState({error3:'This field is required'});
     
    } else if(document.getElementById("eliquis_rdo").checked === true && !$("input[name='opt_eliquis_dosage']:checked").val()){
      this.setState({error1:''});
      this.setState({error2:''});
      this.setState({error3:''});
     this.setState({error4:'This field is required'});

    } else if(document.getElementById("edxo_rdo").checked === true && !$("input[name='opt_edxo_dosage']:checked").val()){
    ...
    } else if(document.getElementById("15_mg_once2").checked === true && !$("input[name='opt_xarelto_dosage_15mg_daily']:checked").val()){
 ...
 this.setState({error8:'This field is required'});
  ...
  this.setState({error8:'',error7:'',error6:'',error5:'',error4:'',error3:''});

this.setState({error9:'This field is required'});

} else if(document.getElementById("opt_edxo_dosage_other").checked === true && $('#opt_edxo_dosage_other_textbox').val()=='' ){
  this.setState({error8:'',error7:'',error6:'',error5:'',error4:'',error3:'',error9:''});
...
page8(){
  ...
  if(document.getElementById("pradaxa_rdo").checked === true){
    ...
    edoxabon_dosage_time:this.state.q5_ans
    }
...
pradaxa_rdo(){
  this.setState({
    q1_ans:'',q1_ans_dosage:'',
  q2_ans:'',q2_ans_dosage:'',
  q3_ans:'',q3_ans_dosage:'',
  q4_ans:'',q4_ans_dosage:'',
  q5_ans:'',
  q2_ans_dosage_timing:'',
  q2_ans_dosage_meal_taken:''
});
...
<div className="centered"><ReactSpinner type="border" color="bg-primary" size="5"  /></div> :''
  ...
  <div className="bg-light blue-box"> {/* white box */}
    ...
    <form className="p-5" action="#!">
      <p className="blue"><b>Are you currently on any of these drugs?</b></p>
      <label className="radio-inline blue">Pradaxa (Dabigatran)</label>
      <input type="radio" name="optradio" className="pull-right" id="pradaxa_rdo"
       onClick={this.pradaxa_rdo} 
       onChange={(e)=>this.setState({q1_ans:'Pradaxa (Dabigatran)'})}
        
           />
      <div id="pradaxa_dosage">
        <p className="blue"><b>Dosage</b></p>
        <label className="radio-inline blue">75 mg twice daily</label>
        <input type="radio" name="opt_pradaxa_dosage" className="pull-right"
      ...
      <input type="radio" name="optradio" className="pull-right" id="xarelto_rdo" onChange={(e)=>this.setState({q2_ans:'Xarelto (Rivaroxaban)'})}
      ...
      <div id="xarelto_dosage">
        ...
        <input type="radio" name="opt_xarelto_dosage" id="15_mg_once1" className="pull-right" 
           onChange={(e)=>this.setState({q2_ans_dosage:'15 mg twice daily'})}
           
           />
        <br />
        <label className="radio-inline blue">15 mg once daily</label>
        <input type="radio" name="opt_xarelto_dosage" id="15_mg_once2" className="pull-right" 
        ...
        <div id="opt_xarelto_dosage_15mg_daily_div">
          ...
          <input type="radio" name="opt_xarelto_dosage_15mg_daily" id className="pull-right"    onChange={(e)=>this.setState({q2_ans_dosage_timing:'Taken in the morning'})} />
          <br />
          &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<label className="radio-inline blue">Taken later in the day</label>
          <input type="radio" name="opt_xarelto_dosage_15mg_daily" id className="pull-right"     onChange={(e)=>this.setState({q2_ans_dosage_timing:'Taken later in the day'})}/>
        
          <div className="text-danger"> {this.state.error6!=='' ? this.state.error6: ''}</div>  <br />
        </div>
        <label className="radio-inline blue">20 mg twice daily</label>
        <input type="radio" name="opt_xarelto_dosage" id="20_mg_once1" className="pull-right"     onChange={(e)=>this.setState({q2_ans_dosage:'20 mg twice daily'})}/>
        <br />
        <label className="radio-inline blue">20 mg once daily</label>
        <input type="radio" name="opt_xarelto_dosage" id="20_mg_once2" className="pull-right"    onChange={(e)=>this.setState({q2_ans_dosage:'20 mg once daily'})} />
        <br />
  
        <div id="opt_xarelto_dosage_20mg_daily_div">
          &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<label className="radio-inline blue">Taken in the morning</label>
          <input type="radio" name="opt_xarelto_dosage_20mg_daily" id="opt_xarelto_dosage_20mg_daily1" className="pull-right"     onChange={(e)=>this.setState({q2_ans_dosage_timing:'Taken in the morning'})}/>
          <br />
          &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<label className="radio-inline blue">Taken later in the day</label>
          <input type="radio" name="opt_xarelto_dosage_20mg_daily" id="opt_xarelto_dosage_20mg_daily2" className="pull-right" onChange={(e)=>this.setState({q2_ans_dosage_timing:'Taken later in the day'})}/>
        </div>
        ...
        <input type="text" id="bridging2" className="form-control mb-4 transparent-custom-input" placeholder="If once daily, please specify meal taken with" 
        defaultValue={this.state.q1_ans_dosage_meal_taken} onChange={(e)=>this.setState({q2_ans_dosage_meal_taken:e.target.value})}
        />
      ...
      <input type="radio" name="optradio" className="pull-right" id="eliquis_rdo"  
      ...
      <div id="eliquis_dosage">
        ...
        <input type="radio" name="opt_eliquis_dosage" className="pull-right" onChange={(e)=>this.setState({q3_ans_dosage:'5 mg twice daily'})}/>
        <br />
        <label className="radio-inline blue">2.5 mg twice daily</label>
        <input type="radio" name="opt_eliquis_dosage" className="pull-right" onChange={(e)=>this.setState({q3_ans_dosage:'2.5 mg twice daily'})}/>
        <br />
        ...
        {/* input */}
        <input type="text" id="bridging4" className="form-control mb-4 transparent-custom-input" placeholder="Please specify if other"    onChange={(e)=>this.setState({q3_ans_dosage:e.target.value})} />
      ...
      <input type="radio" name="optradio" className="pull-right" id="edxo_rdo"  onChange={(e)=>this.setState({q4_ans:'Edoxabon (Lixiana)'})}
      ...
      <div id="edxo_dosage">
        ...
        <input type="radio" name="opt_edxo_dosage" className="pull-right"  onChange={(e)=>this.setState({q4_ans_dosage:'60 mg once daily'})}/>
        <br />
        <label className="radio-inline blue">30 mg once daily</label>
        <input type="radio" name="opt_edxo_dosage" className="pull-right"  onChange={(e)=>this.setState({q4_ans_dosage:'30 mg once daily'})}/>
        <br />
        <label className="radio-inline blue">15 mg once daily</label>
        <input type="radio" name="opt_edxo_dosage" className="pull-right"  onChange={(e)=>this.setState({q4_ans_dosage:'15 mg once daily'})}/>
        <br />
        <label className="radio-inline blue">Other</label>
        <input type="radio" name="opt_edxo_dosage" id="opt_edxo_dosage_other" className="pull-right" onChange={(e)=>this.setState({q4_ans_dosage:'Other'})}/>
        {/* input */}
        <input type="text" id="opt_edxo_dosage_other_textbox" className="form-control mb-4 transparent-custom-input" placeholder="Please specify if other" />
       <div className="text-danger"> {this.state.error8!=='' ? this.state.error8: ''}</div>
      ...
      <label className="radio-inline blue">None Of The Above</label>
      <input type="radio" name="optradio" className="pull-right" id="opt_none"  onChange={(e)=>this.setState({q5_ans:'None Of The Above'})}
