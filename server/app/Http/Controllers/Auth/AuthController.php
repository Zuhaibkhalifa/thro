...
class AuthController extends Controller
{
    ...
    public function login(LoginRequest $request)
    {
        ...
        if(!$user) return response()->json(['error' => 'User not found.'], 200);

        // Account verification validation
      //  if(config('url.account_verify')){
            if(!$user->email_verified_at) return response()->json(['error' => 'Account must be verified'], 200);
     //   }
        // Account Validation
        if (!(new BcryptHasher)->check($request->input('password'), $user->password)) {
            // Return Error message if password is incorrect
            return response()->json(['error' => 'Email or password is incorrect. Authentication failed.'], 200);
        }
